...
# Note: Removed pygame and os imports
...
# --- Detection Constants ---
# <<< CHANGED: Lowered threshold to fix false positives >>>
EYE_AR_THRESH = 0.19 
# <<< CHANGED: Reduced frames for better responsiveness >>>
...
while True:
    ...
    if results_face_mesh.multi_face_landmarks:
        ...
        # <<< ADDED: Uncomment this line to debug your EAR value >>>
        # print(f"Current EAR: {avg_ear:.2f}")
        ...
        if head_pose_angles:
            ...
            # <<< ADDED: Uncomment this line to debug your YAW value >>>
        ...
        if results_hands.multi_hand_landmarks:
            h, w, _ = frame_shape
            try:
                # Get key face landmarks for proximity checks
                ...
                # Define key covering landmarks (tips and main knuckles)
                ...
                for hand_landmarks in results_hands.multi_hand_landmarks:
                    ...
                    # Covered Checks Loop - Check only key "covering" landmarks
                    for lm_idx in HAND_COVERING_INDICES:
                        try:
                            ...
                            # Stricter Y-position check (between nose and chin)
                            ...
                            # Stricter Y-position check (between eyebrows and nose)
                            if (left_eye_dist < EYES_COVERED_THRESH_DISTANCE or right_eye_dist < EYES_COVERED_THRESH_DISTANCE) and (eyebrow_y < hand_lm_y < nose_tip_y):
                                ...
                                break # Found an eye-covering point, stop checking landmarks

                        except IndexError:
                            logger.warning(f"Hand landmark index {lm_idx} out of range.")
                    
                    if is_eyes_covered: 
                        break # Found an eye-covering hand, stop checking other hands
    ...
    if hands_detected:
        ...
        if not specific_hand_alert_active and not head_distraction_active: 
            if not HANDS_OFF_WHEEL_ALERT_TRIGGERED:
                ...
                # <<< BUG FIX: Corrected typo from WIIEEL to WHEEL >>>
                HANDS_OFF_WHEEL_ALERT_TRIGGERED = True
        else: # Specific hand/head alert IS active, so suppress general "hands off"
            HANDS_OFF_WHEEL_ALERT_TRIGGERED = False
